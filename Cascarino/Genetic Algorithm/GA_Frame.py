...
def generate_genome(section_data: {List[List]}) -> Genome:
    return [random.randint(0,len(section_list)-1) for section_list in section_data]

def initialize_population(population_size: int, section_data: List[List]) -> Population:
...
def get_min_weights(weights, max_design_ratios) -> float:
    ...
    print(min_weight_all, min_weight_feas)
...
for generation in range(generations):
    ...
    data = [run_structural_analysis(genome = genome, section_lib=section_data) for genome in population]

    max_design_ratios = []
    max_design_ratios = find_max_design_ratios(data=data)

    weights = []
    weights = [calculate_weights(genome, section_data, length_per_sections) for genome in population]
